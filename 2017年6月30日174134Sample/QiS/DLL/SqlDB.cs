...
namespace DAL
{
    public class SqlDB
    {
        ...
        /// <param name="sqlStr">true表示执行成功,false表示执行失败</param>
        /// <returns></returns>
        public bool ExecSql(string sqlStr, SqlParameter[] pms)
        {
            try
            {
                int i;
        ...
        /// <summary>
        /// 带参数查询，将查询结果以DataTable类型返回，如果执行的是各种更新语句，则返回值无意义。
        /// </summary>
        /// <param name="selectSql"></param>
        /// <returns>查询结果</returns>
        public DataTable FillDt_para(string selectSql, SqlParameter[] para)
        {
            //DataTable dt = new DataTable();
            //dA = new SqlDataAdapter(selectSql, conn);
            //dA.Fill(dt);
            ...
            DataTable dt = new DataTable();
            conn.Open();
            SqlCommand sc = new SqlCommand(selectSql, conn);
            try
            {
                foreach (SqlParameter item in para)
                {
